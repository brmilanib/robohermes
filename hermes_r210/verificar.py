...
assert len(manifest['cases'])==8 and len({c['name'] for c in manifest['cases']})==8
...
# Presets: mesmo conjunto de inputs; InpCase varia dentro de 1..8.
assert len(list((ROOT/'presets').glob('*.set')))==11
...
for path in (ROOT/'presets').glob('*.set'):
    ...
    assert 1<=int(start)<=int(end)<=8 and int(step)==1,path.name
...
validation=dict(version='2.10',source_sha256=manifest['source_sha256'],cases=8,presets=11,statistics_fields=116,
    ...
    note='Casos 4/5/6/7/8 sao hipoteses NAO validadas. Exigem backtest MT5 com custos realistas e '
