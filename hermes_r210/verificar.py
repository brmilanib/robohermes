...
tests=['test_core','test_execution','test_donchian','test_hermes','test_entry',
       'test_pivot','test_pivot_entry','test_pivot_adapter','test_quickharvest']
...
assert len(manifest['cases'])==5 and len({c['name'] for c in manifest['cases']})==5
...
for path in (ROOT/'presets').glob('*.set'):
    ...
    assert 1<=int(start)<=int(end)<=5 and int(step)==1,path.name
...
validation=dict(version='2.10',source_sha256=manifest['source_sha256'],cases=5,presets=6,statistics_fields=116,
    frozen_cores_identical_to_r200=True,changed_files=changed,added_files=['QuickHarvestCore.mqh'],
    local_tests=results,native_MQL5_compilation=False,native_MT5_backtests=False,
    note='Casos 4/5 sao hipoteses NAO validadas. Exigem backtest MT5 com custos realistas e '
