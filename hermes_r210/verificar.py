...
tests=['test_core','test_execution','test_donchian','test_hermes','test_entry',
       'test_pivot','test_pivot_entry','test_pivot_adapter','test_quickharvest','test_dd_throttle','test_withdrawal','test_ema_cross']
...
assert len(manifest['cases'])==9 and len({c['name'] for c in manifest['cases']})==9
...
# Presets: mesmo conjunto de inputs; InpCase varia dentro de 1..9.
assert len(list((ROOT/'presets').glob('*.set')))==12
...
for path in (ROOT/'presets').glob('*.set'):
    ...
    assert 1<=int(start)<=int(end)<=9 and int(step)==1,path.name
...
validation=dict(version='2.10',source_sha256=manifest['source_sha256'],cases=9,presets=12,statistics_fields=116,
    frozen_cores_identical_to_r200=True,changed_files=changed,
    added_files=['QuickHarvestCore.mqh','DDThrottleCore.mqh','WithdrawalCore.mqh','EMACrossCore.mqh'],
    local_tests=results,native_MQL5_compilation=False,native_MT5_backtests=False,
    note='Casos 4/5/6/7/8/9 sao hipoteses NAO validadas. Exigem backtest MT5 com custos realistas e '
