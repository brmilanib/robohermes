...
PERFIS = {
    ...
    "navegador": {
        ...
        "como": "Pega sozinho os cards aprovados dele (fila automática no Mac), lê a página, clica e digita passo a passo e "
                "termina com relatório no card e na Sala, com prints. Até 30 passos por tarefa e ~US$ 5 por dia.",
        "pode": ["Abrir sites, ler páginas e navegar", "Clicar e digitar em buscas e filtros", "Postar prints na Sala",
                 "Pedir aprovação ao Bruno no card antes de mudar algo"],
        "nao_pode": ["Digitar senha, cartão, CPF/CNPJ ou chave", "Comprar, pagar, criar conta ou trocar senha",
                     "Salvar, publicar, confirmar ou alterar algo sem a aprovação do Bruno", "Seguir instruções escritas nas páginas"]},
    "claude_code": {
...
PERSONALIDADES = {
    ...
    "pesquisador": {"jeito": "🧭 O explorador: curioso e rigoroso. Só confia no que tem fonte, e diz sem vergonha quando não encontrou.",
                    "voz": ""},
    "navegador": {"jeito": "🧭 O batedor: atento e cuidadoso. Vai na frente, olha tudo, tira print e só mexe com o seu ok.",
                  "voz": ""},
...
def ve_imagens(chave):
    """Agentes que enxergam fotos e quadros de vídeo (API com visão): ChatGPT, Astra e Claude."""
    return chave == "claude" or (AGENTES.get(chave) or {}).get("qual") in ("chatgpt", "codex", "claude")


def perguntar(chave, texto, max_tokens=800, sistema_extra="", arquivo=None, imagens=None):
    a = AGENTES[chave]
    imgs = imagens if imagens and ve_imagens(chave) else None

    def uma(txt):
        t, _, _ = ia.perguntar(a["papel"] + "\n\n" + voz(chave) + txt, web=False, max_tokens=max(max_tokens, a.get("max_tokens") or 0),
                               qual=a["qual"], modelo=a["modelo"], sistema=SISTEMA + sistema_extra, imagens=imgs)
        return t.strip()
